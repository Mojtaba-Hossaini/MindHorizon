...
function LikeOrDisLike(postId, isLiked) {
    $.ajax({
        url: "/Home/LikeOrDisLike?postId=" + postId + "&&isLike=" + isLiked
    }).done(function (result) {
...
function Bookmark(postId) {
    $.ajax({
        url: "/Home/BookmarkPost?postId=" + postId
    }).done(function (result) {
        if (result === true) {
            ...
            $("#pills-tab").after("<p class='alert alert-danger'>برای بوکمارک کردن مطلب ابتدا باید وارد سایت شوید.</p>");
