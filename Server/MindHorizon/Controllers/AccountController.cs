...
using MindHorizon.ViewModels.Account;
...
namespace MindHorizon.Controllers
{
    public class AccountController : Controller
    {
        ...
        private const string BookmarkNotFound = "مطلب بوکمارک شده یافت نشد.";
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        await _emailSender.SendEmailAsync(viewModel.Email, "تایید حساب کاربری - سایت افق فکر", $"<div dir='rtl' style='font-family:tahoma;font-size:14px'>لطفا با کلیک روی لینک رویه رو حساب کاربری خود را فعال کنید.  <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>کلیک کنید</a></div>");
        ...
        public async Task<IActionResult> Profile()
        {
            ...
            return View(new UserPanelViewModel(user, await _uw.PostRepository.GetUserBookmarksAsync(userId)));
        }
        ...
        [HttpGet, AjaxOnly]
        public IActionResult DeleteBookmark(int userId, string postId)
        {
            if (userId == 0 || !postId.HasValue())
                ModelState.AddModelError(string.Empty, BookmarkNotFound);
            else
            {
                var bookmark = _uw.BaseRepository<Bookmark>().FindByConditionAsync(b => b.PostId == postId && b.UserId == userId).Result.FirstOrDefault();
                if (bookmark == null)
                    ModelState.AddModelError(string.Empty, BookmarkNotFound);
                else
        ...
        public async Task<IActionResult> DeleteBookmarkConfirmed(Bookmark model)
        {
            ...
            {
                var bookmark = _uw.BaseRepository<Bookmark>().FindByConditionAsync(b => b.UserId == model.UserId && b.PostId == model.PostId).Result.FirstOrDefault();
                if (bookmark == null)
                    ModelState.AddModelError(string.Empty, BookmarkNotFound);
                else
                {
                    ...
                    await _uw.Commit();
                    return PartialView("_Bookmarks", await _uw.PostRepository.GetUserBookmarksAsync(model.UserId));
                }
            }
            return PartialView("_DeleteConfirmation");
        }
